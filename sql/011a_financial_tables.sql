-- =====================================================
-- ITAD ERP Guatemala - Módulo Financiero (PARTE 1)
-- EJECUTAR PRIMERO: Tablas e Índices
-- =====================================================
...
-- 2.1. Asegurar columnas existentes si la tabla expense_ledger ya existía sin todas las columnas
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS expense_type TEXT;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS reference_number TEXT;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS amount NUMERIC(12,2);
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS currency TEXT DEFAULT 'GTQ';
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS quantity INTEGER DEFAULT 1;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS unit_cost NUMERIC(10,2);
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS expense_date DATE DEFAULT CURRENT_DATE;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'pending';
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS created_by UUID;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS settlement_id UUID;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS work_order_id UUID;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS asset_id UUID;
ALTER TABLE expense_ledger ADD COLUMN IF NOT EXISTS batch_id UUID;

-- 2.2. Agregar constraint CHECK para expense_type si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conrelid = 'expense_ledger'::regclass 
            AND conname = 'expense_ledger_expense_type_check'
    ) THEN
        ALTER TABLE expense_ledger 
            ADD CONSTRAINT expense_ledger_expense_type_check 
            CHECK (expense_type IN ('acquisition','logistics','parts','labor','data_wipe','storage','marketing','commission','other'));
    END IF;
END$$;

-- 2.3. Asegurar NOT NULL donde aplica (establecer valores por defecto primero)
UPDATE expense_ledger SET expense_type = COALESCE(expense_type, 'other');
UPDATE expense_ledger SET description = COALESCE(description, 'Gasto');
UPDATE expense_ledger SET amount = COALESCE(amount, 0);
UPDATE expense_ledger SET status = COALESCE(status, 'pending');
ALTER TABLE expense_ledger ALTER COLUMN expense_type SET NOT NULL;
ALTER TABLE expense_ledger ALTER COLUMN description SET NOT NULL;
ALTER TABLE expense_ledger ALTER COLUMN amount SET NOT NULL;
...
SELECT 'PARTE 1 COMPLETADA: Tablas creadas correctamente' AS status;
